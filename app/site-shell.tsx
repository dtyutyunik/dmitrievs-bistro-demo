...
import { BASE_PATH, DIRECTIONS, DOORDASH, INSTAGRAM, LOGO, PHONE } from "./site-data";
...
export function SiteHeader({ active }: { active?: "home" | "menu" | "contact" }) {
  return <><div className="design-switch" aria-label="Choose site design"><a href={`${BASE_PATH}/concept/`}>1</a><a className="active" href={`${BASE_PATH}/`}>2</a></div><header className="site-header">
    ...
    <details className="mobile-nav"><summary aria-label="Open navigation"><Menu/></summary><div><a href={`${BASE_PATH}/`}>Home</a><a href={`${BASE_PATH}/our-menu/`}>Our Menu</a><a href={`${BASE_PATH}/contact-us/`}>Contact Us</a><a href={PHONE}>Call (347) 365-7552</a><a href={DOORDASH}>Order on DoorDash</a></div></details>
  </header></>;
}
